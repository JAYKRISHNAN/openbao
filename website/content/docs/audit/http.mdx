...
- `request_timeout` `(string: "")` - The maximum duration to wait for the audit
  log POST before it is cancelled, expressed as a Go duration (e.g. `5s`).
  Defaults to no timeout: the request is bounded only by the caller's context.
